...
export const POSITION_CONFIGS: Record<PositionKey, PositionConfig> = {
  video_content: {
    ...
    fields: [
      NAME_FIELD,
      PERF_PERSONAL("季度累计实际成交金额(万元)", "季度累计实际成交金额，单位：万元", 500),
    ],
    extraNotes: [
  ...
  cross_border_ops: {
    ...
    fields: [
      ...
      PERF_PERSONAL("个人季度月均销售额(万元)", "个人季度月均销售额，单位：万元。管培生/助理可留空", 320),
  ...
  business_bd: {
    ...
    fields: [
      ...
      PERF_PERSONAL("个人季度月均销售额(万元)", "个人季度月均销售额，单位：万元", 250),
    ],
    extraNotes: [
      "依据：总裁办〔2025〕43号",
      "业绩 < 10 万：不在评级区间，需谈薪/人工处理",
  ...
  mall_ops: {
    ...
    fields: [
      ...
      PERF_PERSONAL("个人季度月均销售额(万元)", "个人季度月均销售额，单位：万元", 600),
      { key: "perf_team", label: "团队季度月均销售额(万元)", type: "number",
        required: { when: "职级=组长 时必填" }, unit: "万元",
  ...
  product_ops: {
    ...
    fields: [
      ...
      PERF_PERSONAL("个人季度月均销售额(万元)", "个人季度月均销售额，单位：万元。管培生/助理可留空", 300),
      { key: "education", label: "学历", type: "enum",
        required: { when: "职级=管培生 时必填" }, enum: EDU_VALUES,
  ...
  jd_product_ops: {
    ...
    label: "京东产品运营岗",
    shortLabel: "京东运营",
    description: "按季度月均净销售额单表评级；区间左开右闭；专家档 (>1000万) 固定 45000 元。",
    color: "#cf1322",
    fields: [
      NAME_FIELD,
      { key: "level", label: "职级", type: "enum", required: "always",
        enum: ["专员", "管培生", "助理"],
        comment: "必填：专员/管培生/助理", example: "专员" },
      PERF_PERSONAL("个人季度月均销售额(万元)",
        "季度月均净销售额（净销售额 = 成交金额 − 优化金额 − 推广金额），单位：万元。管培生/助理可留空", 300),
      { key: "education", label: "学历", type: "enum",
        required: { when: "职级=管培生 时必填" }, enum: EDU_VALUES,
        comment: "仅管培生填写：普通本科/211/985本科/普通硕士/211/985硕士", example: "普通本科" },
      PROBATION_FIELD,
    ],
    extraNotes: [
      "依据：总裁办〔2026〕10号《京东产品运营岗位薪资评级方案》",
      "净销售额 = 成交金额(京东商智) − 优化金额(部门统计) − 推广金额(京准通)",
      "区间为 左开右闭 (低, 高]，边界值归属可能与直觉不同",
      "专家档 (>1000万) 固定 45000 元（不取百）；其余区间线性插值后四舍五入取百",
      "助理：谈薪制，系统不评定",
      "管培生（毕业 ≤1 年）按学历定薪：本科 6000/7000，硕士 8000/9000",
      "试用期 = 转正后标准 × 80%（取百）",
      "适用范围：京东平台专员级产品运营，不含店管",
      "京东业绩占个人总业绩不足 80% 时不适用本方案，请部门负责人审核",
      "新老品承接：月销 ≥10 万的链接交接 3 个月内，交接前后各承接 50% 业绩（线下手算）",
      "本方案自 2026 年 7 月 1 日起生效（数据取值：2026 年 4-6 月）",
    ],
  },
  ...
  livestream_ops: {
    ...
    label: "直播运营/主播组长岗",
    shortLabel: "直播运营",
    description: "按季度月均净销售额评定个人薪资，人均净产值叠加管理薪资；直播运营组长/主播组长保留原职级名。",
    color: "#ff7a45",
    fields: [
      NAME_FIELD,
      { key: "level", label: "职级", type: "enum", required: "always",
        enum: ["直播运营专员", "直播运营组长", "主播组长"],
        comment: "必填：直播运营专员 / 直播运营组长 / 主播组长（三者共用同一评级表）",
        example: "直播运营专员" },
      { key: "perf_personal", label: "季度月均净销售额(万元)", type: "number",
        required: "always", unit: "万元",
        comment: "个人季度月均净销售额，单位：万元", example: 150 },
    ...
    extraNotes: [
      ...
      "直播运营专员：岗位评定按业绩区间命中（初级/中级/高级/专家直播运营）",
      "直播运营组长 / 主播组长：岗位评定保留原职级名称，薪酬计算规则与直播运营专员相同",
  ...
  video_ops: {
    ...
    label: "视频运营岗",
    shortLabel: "视频运营",
    description: "按季度月均净销售额评定个人薪资，人均净产值叠加管理薪资；视频运营组长保留原职级名。",
    color: "#9254de",
    fields: [
      NAME_FIELD,
      { key: "level", label: "职级", type: "enum", required: "always",
        enum: ["视频运营专员", "视频运营组长"],
        comment: "必填：视频运营专员 / 视频运营组长（两者共用同一评级表）",
        example: "视频运营专员" },
      { key: "perf_personal", label: "季度月均净销售额(万元)", type: "number",
        required: "always", unit: "万元",
        comment: "个人季度月均净销售额，单位：万元", example: 250 },
      { key: "per_capita_value", label: "人均净产值(万元)", type: "number",
        required: { when: "对人均净产值负责才需填" }, unit: "万元",
        comment: "可选：填了就叠加管理薪资（不区分职级）；不对人均净产值负责的人员可留空",
        example: 50 },
    ...
    extraNotes: [
      ...
      "视频运营专员：岗位评定按业绩区间命中（初级/中级/高级/专家视频运营）",
      "视频运营组长：岗位评定保留原职级名称，薪酬计算规则与视频运营专员相同",
    ],
  },
};
...
export const POSITION_LIST: PositionConfig[] = [
  ...
  POSITION_CONFIGS.jd_product_ops,
